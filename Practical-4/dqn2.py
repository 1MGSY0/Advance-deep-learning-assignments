...
if __name__ == "__main__":
    ...
    checkpoint_path = "runs/ALE/Breakout-v5__DQN_Breakout__1__1748204243/DQN_Breakout_checkpoint_10000000.pt"
    ...
    if resume_training and os.path.exists(checkpoint_path):
        ...
        target_network.load_state_dict(q_network.state_dict())
        global_step = 10_000_000
        warmup_active = True
        print(f"Resumed training from checkpoint: {checkpoint_path} at step {global_step}")
    else:
        global_step = 0
        warmup_active = False
    ...
    try:
        for step in range(global_step, params.total_timesteps):
            # Here we get epsilon for our epislon greedy.
            evaluation_mode = resume_training and global_step < 10_000_000 + WARMUP_STEPS
            epsilon = 0.0 if evaluation_mode else linear_schedule(params.start_e, params.end_e, params.exploration_fraction * params.total_timesteps, global_step)
                ...
                if episode_counter % num_life == 0:
                    ...
                    if env_episode_counter % env_log_interval == 0:
                        ...
                        log_line = (
                            ...
                            f"| Return: {running_return:.2f} | Elapsed: {elapsed_minutes:.2f} min"
                        )
                        print(log_line, flush=True)
                        with open("training_log_10mil.txt", "a") as f:
                            f.write(log_line + "\n")
            ...
            # Training 
            if (
                global_step > params.learning_starts and 
                (not warmup_active or global_step > 10_000_000 + WARMUP_STEPS)
            ):
